...
use plato_core::input::{DeviceEvent, PowerSource, ButtonCode, ButtonStatus, VAL_RELEASE, VAL_PRESS, EVENT_BUTTONS, EVENT_TOUCH_SCREEN, EVENT_WACOM, InputFilterCommand};
...
pub fn run() -> Result<(), Error> {
    ...
    // Migrate old env to new env in libremarkable
    if let Ok(content) = env::var("PLATO_DISABLE_BUILTIN_SWTFB_CLIENT") {
        if content.eq_ignore_ascii_case("true") || content.eq_ignore_ascii_case("yes") || content == "1" {
            env::set_var("LIBREMARKABLE_FB_DISFAVOR_INTERNAL_RM2FB", "1");
        }
    }

    let mut fb = Box::new(RemarkableFramebuffer::new().context("can't create framebuffer")?);
