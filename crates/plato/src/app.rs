...
use plato_core::input::{DeviceEvent, PowerSource, ButtonCode, ButtonStatus, VAL_RELEASE, VAL_PRESS, InputFilterCommand};
use plato_core::input::{raw_events, device_events, usb_events, display_rotate_event, button_scheme_event};
use plato_core::input::{EVENT_BUTTONS, EVENT_TOUCH_SCREEN, EVENT_WACOM};
...
fn build_context(fb: Box<dyn Framebuffer>) -> Result<Context, Error> {
    ...
    if settings.remarkable.input_sources.is_empty() {
        settings.remarkable.input_sources =
            plato_core::settings::RemarkableSettings::default().input_sources;
